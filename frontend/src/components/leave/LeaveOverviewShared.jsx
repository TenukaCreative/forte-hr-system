...
export function SummaryCard({ label, count, sub, Icon, color, iconBg }) {
  return (
    <div style={{
      position: 'relative',
      overflow: 'hidden',
...
// ── Requests table (HR: /leaves/all · Senior: /leaves/team) ───
export function RequestsTable({ endpoint, badgeLabel }) {
...
// ── Approvals (manager Step 1 / final Step 2) ─────────────────
export function ApprovalsView({ kind }) {
...
// ── Team Leave (Gantt timeline, view-only) ────────────────────
// scope="all"  → org-wide approved leaves (HR)    → /leaves/all
// scope="team" → direct reports' approved leaves  → /leaves/team-approved
export function TeamLeaveView({ scope }) {
  const [teamLeaves, setTeamLeaves] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedRequest, setSelectedRequest] = useState(null);

  useEffect(() => {
    const endpoint = scope === 'all' ? '/leaves/all' : '/leaves/team-approved';
    api.get(endpoint)
...
// ── Plans table (HR: /leave-plans/all · Senior: /leave-plans/team) ─────
export function PlansTable({ endpoint }) {
